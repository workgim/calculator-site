...
- 진행 상황(2026-09-11): 1~12단계 완료. **계산기 16개** (추천 순서 1~12 + Tier 2~3: 날짜/평↔㎡/글자수/기초대사량).
  카테고리 5개(finance/realestate/health/date/living).
  인프라: 금액칸 콤마·증감 버튼, 관련 계산기 내부 링크, sitemap lastmod, 커스텀 도메인,
  GA4(`G-HC8HX39ETH`, 표준 async 태그 — 2026-09-10 수집 검증), about/contact/privacy, Vitest(`npm test`, 137케이스).
- 2026-09-08 완료: **16개 페이지 콘텐츠·SEO 개편**(H2 섹션 + `<Faq>` + 빌드타임 표 + `related`),
  **디자인 보강**(캔버스/카드 층위·그림자·결과 강조·표 손질) + **다크 모드**(자동 + 헤더 토글).
- 2026-09-11 완료: **디자인 2차**(레퍼런스 `생활계산기.com` 참고) — 아이콘 시스템(`Icon`/`IconBadge`, `icon` 필드),
  헤더 카테고리 드롭다운 내비, 헤더 바 분리, 홈·관련 계산기 아이콘 배지, 본문 섹션 번호 배지 + 표 손질.
- **다음**: 계산기 확대는 **방문자 데이터 보고 판단**(11~12월). 그 사이 재산세·연말정산만 씨앗용. → [01-roadmap.md](./01-roadmap.md)
...
| [30-design-guide.md](./30-design-guide.md) | 색 · 타이포 · 여백 · 컴포넌트 · 아이콘 · 다크 모드 · 내비 드롭다운 토큰 | 작성됨 (2026-09-11 디자인 2차까지 반영) |
