...
| GA4 스크립트 (`gtag/js`) | `Analytics.astro` | 방문 통계 | `site.ts` 의 `gaId` (값 있을 때만). **표준 async 태그**를 `<head>` 에 삽입(파싱 비차단). 쿠키 고지는 `privacy.astro` 가 `gaId` 유무로 분기 |
...
| 주 메뉴 | `<nav aria-label="주요">` | 홈 + 카테고리별 드롭다운(각 `<button>` + 계산기 목록 패널). 데스크톱 hover / 터치·키보드 클릭. `Nav.astro`, 세부는 30-design-guide §6 |
